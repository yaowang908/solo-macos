# excluded-apps Specification

## Purpose

The shared Excluded Apps list: bundle identifiers protected from both of Solo's behaviors (never hidden by Solo Focus, never auto-restored by Smart Restore), managed from the status-menu submenu and the Settings window, consulted at action time only.
