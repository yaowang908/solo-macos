# shortcut-config Specification

## Purpose

The Settings window and Toggle Solo Focus shortcut configuration: recording with detectable-conflict warnings (macOS system shortcuts and Solo's own menus), live rebinding, persistence, and the explicit no-shortcut cleared state with a default-combination hint.
