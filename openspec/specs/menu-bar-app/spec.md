...
Solo's agent-style app lifecycle: a menu bar status item with a state-reflecting icon, the prototype menu (toggle Solo Focus, restore windows, Smart Restore switch, quit), a global toggle hotkey, and a clean quit that restores any active session.
